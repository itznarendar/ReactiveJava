...
//for nonflux WebMvcTest

@RunWith(SpringRunner.class)
@WebFluxTest
public class FluxMonoControllerTest {
//for non flux   private MockMvc mockMvc;
	@Autowired
	...
	Flux<Integer> intgerFlux=	webTestClient.get().uri("/flux")
	.accept(MediaType.APPLICATION_JSON).
	exchange().expectStatus().isOk().returnResult(Integer.class).getResponseBody();
	StepVerifier.create(intgerFlux).expectSubscription().expectNext(1).expectNext(2).expectNext(3).expectNext(4).verifyComplete();
	}
    @Test
